...
class MagnifierControl:
    def __init__(self):
        self.magpath = r"HKEY_CURRENT_USER\SOFTWARE\Microsoft\ScreenMagnifier\\"

    def is_mag_on(self):
        is_on = WindowsRegistry.get_value(self.magpath + "\RunningState")
        return is_on

    def magnifier_control(self, action, amount=None):
        match action:
            case "Zoom In" | "Zoom Out":
                self.mag_level(amount)
            case "Dock":
                self.mag_mode(1)
            case "Full Screen":
                self.mag_mode(2)
    ...
    def mag_mode(self, mode=3):
        exists = WindowsRegistry.query_value(self.magpath + "MagnificationMode")
        if exists:
            WindowsRegistry.set_value(self.magpath + "MagnificationMode", mode, value_type='REG_DWORD')

    def mag_invert(self):
        exists = WindowsRegistry.query_value(self.magpath + "Invert")
        if exists:
            current = WindowsRegistry.get_value(self.magpath + "Invert")
            if current == 1:
                WindowsRegistry.set_value(self.magpath + "Invert", 0, value_type='REG_DWORD')
            if current == 0:
                WindowsRegistry.set_value(self.magpath + "Invert", 1, value_type='REG_DWORD')

    def mag_increments(self, amount):
        exists = WindowsRegistry.query_value(self.magpath + "ZoomIncrement")
        if exists:
            if amount:
                WindowsRegistry.set_value(self.magpath + "ZoomIncrement", amount, value_type='REG_DWORD')

    def mag_level(self, amount, onhold=False):
        themin = 20
        themax = 1000
        current = WindowsRegistry.get_value("HKEY_CURRENT_USER\SOFTWARE\Microsoft\ScreenMagnifier\Magnification")
        exists = WindowsRegistry.query_value(self.magpath + "Magnification")
        if not onhold:
            if exists:
                if amount:
                    if amount > themax:
                        self.mag_on()
                        WindowsRegistry.set_value(self.magpath + "Magnification", themax, value_type='REG_DWORD')
                    else:
                        self.mag_on()
                        WindowsRegistry.set_value(self.magpath + "Magnification", amount, value_type='REG_DWORD')
        if onhold:
            if exists:
                if current < themin:
                    print("less than 20")
                    WindowsRegistry.set_value(self.magpath + "Magnification", 20, value_type='REG_DWORD')
                elif current > themax:
                    try:
                        WindowsRegistry.set_value(self.magpath + "Magnification", 1600, value_type='REG_DWORD')
                    except:
                        pass
                else:
                    try:
                        WindowsRegistry.set_value(self.magpath + "Magnification", current + amount, value_type='REG_DWORD')
                    except:
                        pass

    def text_smoothing(self, switch):
        exists = WindowsRegistry.query_value(self.magpath + "UseBitmapSmoothing")
        if exists:
            if switch == "On":
                WindowsRegistry.set_value(self.magpath + "UseBitmapSmoothing", 1, value_type='REG_DWORD')
            elif switch == "Off":
                WindowsRegistry.set_value(self.magpath + "UseBitmapSmoothing", 0, value_type='REG_DWORD')

    def magnifer_dimensions(self, x=None, y=None, onhold=False):
        exists = WindowsRegistry.query_value(self.magpath + "LensWidth")
        themax = 95
        themin = 20
        if not onhold:
            if x:
                if exists:
                    WindowsRegistry.set_value(self.magpath + "LensWidth", x, value_type='REG_DWORD')
            if y:
                if exists:
                    WindowsRegistry.set_value(self.magpath + "LensHeight", y, value_type='REG_DWORD')

        if onhold:
            if exists:
                if x:
                    current = WindowsRegistry.get_value(self.magpath + "LensWidth")
                    if current < themin:
                        print("less than 20")
                        WindowsRegistry.set_value(self.magpath + "LensWidth", 20, value_type='REG_DWORD')
                    elif current > themax:
                        WindowsRegistry.set_value(self.magpath + "LensWidth", 95, value_type='REG_DWORD')
                    else:
                        WindowsRegistry.set_value(self.magpath + "LensWidth", current + onhold, value_type='REG_DWORD')
                if y:
                    current = WindowsRegistry.get_value(self.magpath + "LensHeight")
                    if current < themin:
                        print("less than 20")
                        WindowsRegistry.set_value(self.magpath + "LensHeight", 20, value_type='REG_DWORD')
                    elif current > themax:
                        WindowsRegistry.set_value(self.magpath + "LensHeight", 95, value_type='REG_DWORD')
                    else:
                        WindowsRegistry.set_value(self.magpath + "LensHeight", current + onhold, value_type='REG_DWORD')
