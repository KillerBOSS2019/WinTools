        ...
        for partition in partitions:
            ...
            if not drives:
                if partition.mountpoint[:2] in thedrivenames:
                    ...
                    #print(the_drive_name, partition.mountpoint[:2], "\nFree:"+get_size(partition_usage.free).replace("GB",""), "\nUsed:"+get_size(partition_usage.used).replace("GB",""), "\nTotal Space:"+get_size(partition_usage.total).replace("GB",""), "\n=======")
                    #print(f"{str_percent}%")
...
def updateStates():
    ...
    if running:
        ...
        if counter%2 == 0:
         ...
         #causing vcom table error then crash on 3rd  TPClient.send(
         #causing vcom table error then crash on 3rd          {
         #causing vcom table error then crash on 3rd              "type":"connectorUpdate",
         #causing vcom table error then crash on 3rd              "connectorId":"pc_Windows-Tools_KillerBOSS.TP.Plugins.VolumeMixer.connectors.APPcontrol|KillerBOSS.TP.Plugins.VolumeMixer.slidercontrol=Master Volume",
         #causing vcom table error then crash on 3rd              "value": str(getMasterVolume())
         #causing vcom table error then crash on 3rd          }
         #causing vcom table error then crash on 3rd      )
            print("STEP 2.2")
            try:
...
def handleSettings(settings, on_connect=False):
    ...
    th=threading.Thread(target=time_booted)
    th.start()
    
    
    m_volume_thread = threading.Thread(target=getMasterVolume2)
    m_volume_thread.start()
    #schedule.every(1).seconds.do(timebooted_loop)
    ## Starting Back Up
    stop_run_continuously = run_continuously()
    return settings
...
def getMasterVolume2():
    ...
    while True:
        ...
        #    print("VALUE ERROR IN GET MASTER VOLUME")
